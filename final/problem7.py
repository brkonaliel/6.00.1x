...
class Family(object):
    ...
    def cousin(self, a, b):
        ...
        if a_node == b_node:
            return (-1, 0)
        elif a_node.is_child(b_node) or a_node.is_parent(b_node):
            return (-1, 1)
        while True:
            if a_node.get_parent():
                a_distance+=1
                a_node = a_node.get_parent()
            if b_node.get_parent():
                b_distance+=1
                b_node = b_node.get_parent()
            if a_node == b_node:
                cousin_type = min(a_distance, b_distance) - 1
                removed = abs(a_distance - b_distance)
                break
        return (cousin_type, removed)
